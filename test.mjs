...
describe("Constants", () => {
  ...
  it("Yallop descriptions are non-empty strings", () => {
    for (const cat of ["A", "B", "C", "D", "E", "F"]) {
      assert.ok(
        typeof YALLOP_DESCRIPTIONS[cat] === "string" && YALLOP_DESCRIPTIONS[cat].length > 0,
      );
    }
  });
...
describe("getMoonPhase structure", () => {
  ...
  it("illumination is in [0, 100]", () => {
    assert.ok(
      phase_march1.illumination >= 0 && phase_march1.illumination <= 100,
  ...
  it("elongationDeg is in [0, 180]", () => {
    assert.ok(
      phase_march1.elongationDeg >= 0 && phase_march1.elongationDeg <= 180,
  ...
  it("prevNewMoon is before reference date", () => {
    assert.ok(
      phase_march1.prevNewMoon < DATE_MARCH_1_2025,
